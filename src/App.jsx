      ...
      <div style={{
        ...
        >
          ...
          <div onClick={() => fileInputRef.current?.click()} style={{
            ...
            <div style={{ fontSize: "14px", color: "#aaa", textAlign: "center" }}>スプライトシートをアップロード</div>
            <div style={{ fontSize: "11px", color: "#555" }}>タップして選択 / ドラッグ＆ドロップ</div>
          </div>
          <div style={{
            ...
            <div style={{ color: "#c44dff", fontWeight: "bold", marginBottom: "4px" }}>💡 v3 新機能</div>
            <div>✨ キャラクター自動検出</div>
            <div>📌 足元固定アンカー（ガタつき防止）</div>
            <div>🎮 開発向けエクスポート</div>
          </div>
        </div>
      </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: "10px", color: "#555", marginLeft: "auto" }}>
          {image.width}×{image.height} | {frames.length}f
        </span>
        <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageUpload} style={{ display: "none" }} />
      ...
      <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        ...
        {activeTab === "preview" && (
          <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
            <div style={{
              ...
              {detecting ? (
                <div style={{ color: "#c44dff", fontSize: "14px" }}>🔍 スプライト検出中...</div>
              ) : (
                <>
                  <div style={{
            ...
            <div style={{
              ...
              <SliderRow label="拡大" value={zoom} onChange={setZoom} min={1} max={10} display={`×${zoom}`} />
            </div>
          ...
          <div style={{ padding: "12px" }}>
            <div style={{ marginBottom: "12px" }}>
              <div style={{ fontSize: "11px", color: "#888", marginBottom: "6px" }}>
                元画像 {splitMode === "auto" && detectedRegions.length > 0 && `(${detectedRegions.length}体検出)`}
              </div>
            ...
            <div style={{ fontSize: "11px", color: "#888", marginBottom: "6px" }}>
              フレーム選択 — {selectedFrames.length}/{frames.length} 選択中
            </div>
            <div style={{
              display: "grid",
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "12px" }}>
              <Btn onClick={() => setSelectedFrames(frames.map((_, i) => i))} color="#4dff88" full>全選択</Btn>
              <Btn onClick={() => setSelectedFrames([])} color="#ff6b6b" full>全解除</Btn>
            </div>
          </div>
        )}
        ...
        {activeTab === "settings" && (
          <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "20px" }}>
            {/* Anchor */}
            <SettingsSection title="📌 アンカー（基準点）">
              <div style={{ fontSize: "11px", color: "#888", marginBottom: "4px" }}>
                フレーム間の位置揃えの基準を設定します
              </div>
              <div style={{ display: "flex", gap: "6px" }}>
                {[
                  { id: "bottom", label: "⬇ 足元固定", desc: "歩行・走行・攻撃に最適" },
                  { id: "center", label: "◎ 中央", desc: "ジャンプ・落下に" },
                  { id: "top", label: "⬆ 頭固定", desc: "特殊用途" },
                ].map((a) => (
            ...
            <SettingsSection title="🔍 分割モード">
              ...
              {splitMode === "auto" && (
                <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "4px" }}>
                  <SliderRow label="感度" value={threshold} onChange={setThreshold} min={180} max={254} display={threshold} />
                  <SliderRow label="最小" value={minSize} onChange={setMinSize} min={5} max={100} display={`${minSize}px`} />
                  <Btn onClick={rerunAutoDetect} color="#4dff88" full>
                    🔄 再検出（{detectedRegions.length}体検出中）
                  </Btn>
                </div>
              )}
              {splitMode === "grid" && (
                <div style={{ marginTop: "4px" }}>
                  <div style={{ display: "flex", gap: "16px" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "11px", color: "#888", marginBottom: "4px" }}>列数</div>
                      <StepperInput value={cols} onChange={setCols} min={1} max={32} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "11px", color: "#888", marginBottom: "4px" }}>行数</div>
                      <StepperInput value={rows} onChange={setRows} min={1} max={32} />
                    </div>
                  </div>
                  {image && (
                    <div style={{ fontSize: "11px", color: "#666", marginTop: "8px" }}>
                      フレームサイズ: {Math.floor(image.width / cols)} × {Math.floor(image.height / rows)}px
                    </div>
                  )}
                  <Toggle label="グリッド表示" checked={showGrid} onChange={setShowGrid} />
                </div>
              )}
            </SettingsSection>

            {/* Background */}
            <SettingsSection title="🎨 背景色">
            ...
            {/* Export */}
            <SettingsSection title="💾 エクスポート">
              <div style={{ fontSize: "11px", color: "#888", marginBottom: "4px" }}>
                選択中: {selectedFrames.length} / {frames.length} フレーム
                {anchor === "bottom" && " | 足元固定済み"}
              </div>
              <div style={{
                ...
                <div>ゲーム開発用の透過PNG出力には</div>
                <div>同梱の <span style={{ color: "#ffb86b" }}>sprite_exporter.py</span> を使用：</div>
                <div style={{
